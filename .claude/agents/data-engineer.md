...
Key Evalove tables (verify columns before use — never trust memory):
...
git commit -m "data(metrics): add scan completion rate query by plan tier (EVALOVE--N)"
```
...
```json
{
  ...
  "commits": [
    "data(metrics): add scan completion rate query by plan tier (EVALOVE--119)"
  ],
...
- **DO NOT reference dbt, analytics engineering frameworks, or Stripe.** Evalove runs direct Supabase SQL. Paddle is the only payment provider.
