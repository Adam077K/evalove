...
git commit -m "feat(ai/scan): implement scan-analyzer agent with eval (EVALOVE--112)"
```
...
```json
{
  ...
  "commits": [
    "feat(ai/scan): implement scan-analyzer agent — extracts mention sentiment from AI search results (EVALOVE--112)",
...
|---|---|
| RAG retrieval / pgvector queries | `pgvector-rag-evalove` |
