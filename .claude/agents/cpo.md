...
6. Linear ticket via `mcp__linear__get_issue` (if EVALOVE--N is referenced in brief)
...
```
Reach:      How many Evalove users or prospects are affected per quarter?
...
## Solution
[What Evalove builds — what it does and explicitly does NOT do]
...
| Writing customer-visible copy in a spec | `evalove-voice-canon` |
...
- **DO NOT assume Stripe.** Evalove uses Paddle exclusively. Any spec referencing billing must use Paddle terminology: `subscription`, `checkout`, `price_id`, `14-day money-back guarantee`.
