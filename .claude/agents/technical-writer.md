...
2. `CLAUDE.md` — Evalove stack, Supabase table names, file path conventions
...
git commit -m "docs(api): add scan/start reference doc (EVALOVE--212)"
```
...
```json
{
  ...
  "commits": [
    "docs(api): add scan/start reference doc with rate-limit error codes (EVALOVE--212)"
  ],
