...
description: "Evalove's locked Linear label vocabulary, sub-ticket creation pattern, and single-synthesis-comment style. Use whenever any CEO or C-suite agent interacts with Linear tickets, creates sub-tickets, or posts comments."
tags: [workflow, evalove-specific, linear, orchestration]
...
```
Evalove — Main (product work)        ← Adam and agents work here
Evalove — War Room (orchestration)   ← Standing Routines post here, separate from product
Evalove — Strategy (board meetings)  ← Board meeting outputs
```
